...
bifurcationPlotSimple(parameter_values = results_batch_1_endstate$`gui_number-herds-dairy`, 
                      ...
                      col = c("darkgreen", "cadetblue", "maroon"),
                      legend_position = c(6, 98),
                      cex = 5)
...
expname <- "sensitivity-param-sweep-permutation_ff_2x"
```
...
Read output:
```{r}
results_batch_2 <- read.csv(paste0(outpath, "/", expname, ".csv"))
```
...
Read output:
```{r}
results_batch_3 <- read.csv(paste0(outpath, "/", expname, ".csv"))
```
...
results_batch_3_endstate <- results_batch_3 %>%
  ...
  summarise(
    ...
    unsustainable_state_mean = mean(as.numeric(`unsustainable-state`))
    ) %>%
  ungroup()
...
tripleRaster_fourParameters(
  ...
  parameter_x_label = "Number of dairy herds",
  parameter_y_label = "Number of transhumance herds",
  variable_1_label = "Number of unsustainable herds (dairy)",
  variable_2_label = "Number of unsustainable herds (transhumance)",
  variable_3_label = "Unsustainable end state",
  plot_scale = 5,
  variable_3_point_number_legend_keys = 2,
  variable_3_print_values = FALSE)
